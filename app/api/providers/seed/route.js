...
const sampleProviders = [
    ...
    },
    {
        ...
        phone: '33-5678-9012', address: 'Av. Hidalgo 240, Centro Zapopan',
        city: 'Zapopan',
        state: 'Jalisco',
        postal_code: '45100',
        latitude: 20.7214,
    ...
    try {
        ...
        for (const provider of sampleProviders) {
            try {
                ...
                errors.push({ provider: provider.business_name, error: error.message });
            }
        }

        return NextResponse.json({
            success: true,
            message: `Inserted ${inserted} providers`,
            errors: errors.length > 0 ? errors : undefined
        });
    } catch (error) {
        return NextResponse.json(
            { error: 'Failed to seed providers', details: error.message },
            { status: 500 }
        );
    }
}
