...
function PostViewContent() {
    ...
    const handleDelete = async () => {
        ...
        if (res.success) {
            router.push(`/communities/view?slug=${post.community_slug}`);
        }
    };
            ...
            <div className="sticky top-0 z-40 bg-green-950/80 backdrop-blur-xl border-b border-white/5">
                <div className="max-w-3xl mx-auto px-4 py-4 flex items-center gap-4">
                    ...
                    <div className="flex-1">
                        ...
                        <Link href={`/communities/view?slug=${post.community_slug}`} className="text-sm text-primary hover:underline">
                            {post.community_name}
                        </Link>
                    </div>
                </div>
