...
function PetViewContent() {
    ...
    const petId = searchParams.get('id'); // Get ID from query param instead of route param
    ...
    async function fetchMedicalRecords() {
        if (!petId) return;
        const recordsRes = await getMedicalRecords(petId);
    ...
    async function fetchData() {
        if (!petId) {
            setErrorMessage('No se proporcionó ID de mascota');
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            if (res.ok) {
                ...
                const petInfoUrl = `${window.location.origin}/public/pet/view?id=${petId}`;
    ...
    const handleDownloadCredential = async () => {
        try {
            const API_BASE = process.env.NEXT_PUBLIC_API_URL || '';
            const res = await fetch(`${API_BASE}/api/og?id=${pet.pet_id}`);
        ...
        <div className="min-h-screen bg-background-dark text-white pb-28 overflow-x-hidden relative">
            ...
            <div className="sticky top-0 z-50 bg-slate-900/80 backdrop-blur-xl border-b border-white/5 animate-slide-up">
                <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
                    ...
                    {pet.isOwner && (
                        <button
                            onClick={() => router.push(`/pets/edit-pet?id=${petId}`)}
                            className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:bg-white/10 transition-colors"
                        >
                            <FaEdit size={18} />
                        </button>
                    )}
                    {!pet.isOwner && <div className="w-10"></div>}
                </div>
            </div>
                        ...
                        <div className="flex -space-x-3 overflow-hidden p-1">
                            {pet.owners && pet.owners.length > 0 ? (
                                <>
                                    ...
                                    <button
                                        onClick={() => router.push(`/pets/edit-pet?id=${pet.pet_id}`)}
